...
pub async fn create_invoice(
    ...
    match invoice {
        ...
        None => Err(InvoiceError::NotFound(&invoice_id).into()),
    }
}
...
pub async fn delete_invoice(
    ...
    if affected != 1 {
        return Err(InvoiceError::NotFound(invoice_id).into());
    }
...
pub async fn update_invoice(
    ...
    if affected == 0 {
        return Err(InvoiceError::InvalidInvoice("invalid invoice").into());
    }
    ...
    match invoice {
        Some(invoice) => Ok(invoice),
        None => Err(InvoiceError::InvalidInvoice("invalid").into()),
    }
}

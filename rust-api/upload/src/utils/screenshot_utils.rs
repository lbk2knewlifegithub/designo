    ...
    match format {
        OutputFormat::JPG => {
            return Ok(tab.capture_screenshot(
                ScreenshotFormat::JPEG(Some(quality.into())),
                None,
                !visible_only,
            )?);
        }
        OutputFormat::PNG => {
            return Ok(tab.capture_screenshot(ScreenshotFormat::PNG, None, !visible_only)?)
        }
        OutputFormat::PDF => return Ok(tab.print_to_pdf(None)?),
    };
}

/// Write image data to a file. A simple wrapper around std::fs::write().
pub fn write_screenshot(output_path: &str, img_data: Vec<u8>) -> Result<(), failure::Error> {

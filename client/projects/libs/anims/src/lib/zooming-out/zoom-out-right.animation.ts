    ...
    group([
      ...
      animate(
        ...
        keyframes([
          ...
          style({ 'transform-origin': 'right center', offset: 0.4 }),
        ])
      ),
    ])
  );
...
export function zoomOutRightAnimation(
  options?: AnimationOptions
): AnimationTriggerMetadata {
  return trigger((options && options.anchor) || 'zoomOutRight', [
    transition(
      '0 => 1',
      [...useAnimationIncludingChildren(zoomOutRight(), options)],
      {
        params: {
...
export function zoomOutRightOnLeaveAnimation(
  options?: AnimationOptions
): AnimationTriggerMetadata {
